...
function normalizeGedcomx(gx) {
  ...
  try {
    ...
    gx?.relationships?.sort((a, b) => relationshipCompareFunction(a, b, gx?.persons));
  } catch (error) {
...
export default function VisualGedcomxDiffer({leftData = null, rightData = null, hideInputs = false, cacheData = true, assertionDefaults = null}) {
  const assertionsContext = React.useContext(AssertionsContext);
  const [assertions, setAssertions] = React.useState(assertionDefaults ? assertionDefaults : assertionsContext.assertions);
  const cachedData = localStorage.getItem(CACHE_KEY) ? JSON.parse(localStorage.getItem(CACHE_KEY)) : null;

  const [leftGxOriginal, setLeftGxOriginal] = React.useState(leftData && leftData.gx ? leftData.gx : cachedData ? cachedData.leftGxOriginal : EMPTY_GEDCOMX);
  const [rightGxOriginal, setRightGxOriginal] = React.useState(rightData && rightData.gx ? rightData.gx : cachedData ? cachedData.rightGxOriginal : EMPTY_GEDCOMX);
  const [leftGx, setLeftGx] = React.useState(leftData && leftData.gx ? leftData.gx : cachedData ? cachedData.leftGx : EMPTY_GEDCOMX);
  const [rightGx, setRightGx] = React.useState(rightData && rightData.gx ? rightData.gx : cachedData ? cachedData.rightGx : EMPTY_GEDCOMX);
  const [finalGx, setFinalGx] = React.useState(getGxIntersection(leftGx, rightGx, assertions));
  const [leftFilename, setLeftFilename] = React.useState(leftData && leftData.filename ? leftData.filename : cachedData ? cachedData.leftFilename : '');
  const [rightFilename, setRightFilename] = React.useState(rightData && rightData.filename ? rightData.filename : cachedData ? cachedData.rightFilename : '');
  ...
  }, [leftGx, rightGx, finalGx, leftFilename, rightFilename, leftGxOriginal, rightGxOriginal, cacheData]);
        ...
        <Stack spacing={1} sx={{marginY: 1}}>
          ...
          <Stack spacing={2} justifyContent='space-around' direction='row'>
            <Stack direction='row' spacing={2}>
              <Button variant='contained' size='large' color='secondary' onClick={() => handleDownload(leftGx, `left.${leftTab === 0 ? 'original' : 'edit'}`)}>{`Download ${leftTab === 0 ? 'Original' : 'Edited'}`}</Button>
              <Button variant='outlined'>
                <Clipboard data-clipboard-text={JSON.stringify(leftGx)} component='div'>{`Copy ${leftTab === 0 ? 'Original' : 'Edited'}`}</Clipboard>
              </Button>
            </Stack>
            <Stack direction='row' spacing={2}>
              <Button variant='contained' size='large' onClick={() => handleDownload(finalGx, 'final')}>Download</Button>
              <Button variant='outlined'>
                <Clipboard data-clipboard-text={JSON.stringify(finalGx)} component='div'>Copy</Clipboard>
              </Button>
            </Stack>
            <Stack direction='row' spacing={2}>
              <Button variant='contained' size='large' color='secondary' onClick={() => handleDownload(rightGx, `right.${rightTab === 0 ? 'original' : 'edit'}`)}>{`Download ${rightTab === 0 ? 'Original' : 'Edited'}`}</Button>
              <Button variant='outlined'>
                <Clipboard data-clipboard-text={JSON.stringify(rightGx)} component='div'>{`Copy ${rightTab === 0 ? 'Original' : 'Edited'}`}</Clipboard>
              </Button>
            </Stack>
          </Stack>
          <Grid container spacing={1}>
            <Grid item xs={4}>
              <Paper variant='outlined'>
                <Tabs value={leftTab} onChange={(event, newValue) => setLeftTab(newValue)} variant='fullWidth'>
                  <Tab label='Original'/>
                  <Tab label='Edited'/>
                </Tabs>
            ...
            <Grid item xs={4}>
              <Paper variant='outlined'>
                <Tabs value={rightTab} onChange={(event, newValue) => setRightTab(newValue)} variant='fullWidth'>
                  <Tab label='Original'/>
                  <Tab label='Edited'/>
                </Tabs>
...
function DiffAccordion({defaultExpanded = false, title, component}) {
  ...
  return (
    <Accordion variant='outlined' defaultExpanded={defaultExpanded} onChange={() => setExpanded(!expanded)}>
      <Tooltip title={`Click to ${tooltipValue}`} placement={"top"} followCursor={true}>
