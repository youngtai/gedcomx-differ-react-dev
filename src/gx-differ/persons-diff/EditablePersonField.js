...
export default function EditablePersonField({field, fieldIndex, person, personIndex}) {
  ...
  const [editFieldType, setEditFieldType] = React.useState(field && field.type ? field.type : '');
  const [editFieldValue, setEditFieldValue] = React.useState(field ? field.values[0].text : '');
  const [hasMatch, setHasMatch] = React.useState(hasMatchingField(person, comparingTo));
