...
export default function EditableFactAttribute({attributeData, fact, factIndex, parentObject, parentObjectIndex, comparingTo, updateData, factTypes}) {
  ...
  const [editFieldValue, setEditFieldValue] = React.useState(attributeData ? attributeData.value : '');
  const [hasMatch, setHasMatch] = React.useState(hasMatchingAttribute(attributeData, fact, parentObject, recordsData.gx.persons, comparingTo, recordsData.comparingToGx.persons, assertions));
        ...
        <Grid item xs={10}>
          <FormControl fullWidth>
            <Select value={editFieldValue} onChange={e => setEditFieldValue(e.target.value)} size='small' sx={{margin: 1}}>
              {Object.keys(factTypes).map(key => <MenuItem key={`type-${key}`} value={factTypes[key]}>{key}</MenuItem>)}
            </Select>
          </FormControl>
        </Grid>
        <Grid item xs={1.2}>
