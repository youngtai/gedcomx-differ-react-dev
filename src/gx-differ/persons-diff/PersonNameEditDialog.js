...
import {Button, Dialog, DialogActions, DialogContent, DialogTitle, Grid, MenuItem, Select, Stack, TextField, Typography} from "@mui/material";
import {NAME_PART_TYPE, PERSON_NAME_TYPE} from "../constants";
import {PaperComponent} from "../Styled";

export default function PersonNameEditDialog({open, setOpen, onClose, nameParts, person}) {
  const [prefix, setPrefix] = React.useState(nameParts?.prefix ? nameParts.prefix.value : '');
  const [given, setGiven] = React.useState(nameParts?.given ? nameParts.given.value : '');
  const [surname, setSurname] = React.useState(nameParts?.surname ? nameParts.surname.value : '');
  const [suffix, setSuffix] = React.useState(nameParts?.suffix ? nameParts.suffix.value : '');
      ...
      <Dialog open={open} onClose={() => onClose(parts, type, person)} PaperComponent={PaperComponent} aria-labelledby="draggable-dialog-title">
        ...
        <DialogActions>
          <Stack direction={"row"} spacing={2}>
            ...
            <Button color={"primary"} onClick={() => onClose(parts, type, person)}>Save</Button>
          </Stack>
        </DialogActions>
      </Dialog>
  )
